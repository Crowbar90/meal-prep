...
Weekly prep schedule generated by the Meal Prep Optimizer. Created when `MealPlanFinalized` triggers the cooking-optimization workflow.
...
| `week_start_date` | DATE | NOT NULL | Monday of the prep week |
| `status` | VARCHAR(20) | DEFAULT 'draft' | `draft`, `feasibility_checked`, `finalized`, `archived` |
| `workflow_id` | UUID | | OpenClaw workflow that generated this |
| `version` | INT | DEFAULT 1 | Optimistic concurrency |
| `feasibility_violations` | JSONB | | Last validator result (nullable) |
| `created_at` | TIMESTAMPTZ | DEFAULT NOW() | |
| `updated_at` | TIMESTAMPTZ | DEFAULT NOW() | |
...
Individual batch prep tasks within a schedule. Each task maps one recipe to a specific day, meal, and time window.

| Column | Type | Constraints | Description |
|--------|------|-------------|-------------|
| `id` | UUID | PK | Unique identifier |
| `prep_schedule_id` | UUID | FK → prep_schedules, ON DELETE CASCADE | Parent schedule |
| `recipe_id` | UUID | FK → recipes | Recipe being prepped |
| `day_of_week` | VARCHAR(10) | NOT NULL | Prep day |
| `meal_type` | VARCHAR(20) | NOT NULL | Target meal type |
| `batch_size_servings` | INT | NOT NULL | Servings this batch produces |
| `earliest_start_offset_minutes` | INT | | Minutes from week_start_date |
| `latest_finish_offset_minutes` | INT | | Hard deadline |
| `equipment_ids` | text[] | DEFAULT '{}' | Required equipment |
| `steps` | JSONB | | Ordered prep steps |
| `assigned_to_slot_ids` | UUID[] | DEFAULT '{}' | Links to meal_slots.id |
| `created_at` | TIMESTAMPTZ | DEFAULT NOW() | |

**Indexes:**
- Composite index on `(prep_schedule_id, day_of_week, meal_type)`
...
Columns that are **flat lists of scalars** (allergens, tags, equipment, dietary restrictions, preferred supermarkets, `equipment_ids`, `assigned_to_slot_ids`) are stored as PostgreSQL `text[]` / `UUID[]` rather than JSONB arrays. Set-membership queries (`WHERE 'peanuts' = ANY(allergens)`, `WHERE tags && ARRAY['vegan','quick']`) are directly indexable with a GIN on the array and avoid the JSONB decode cost. JSONB is reserved for **structured documents** with mixed types or named fields: `nutrition_per_100g`, `nutrition_goals`, `food_preferences`, `meal_plans.nutrition_summary`, `prep_tasks.steps`, `prep_schedules.feasibility_violations`, and the decision-context payloads on `decision_events`. The split keeps query patterns simple and GIN-index coverage explicit per column.
